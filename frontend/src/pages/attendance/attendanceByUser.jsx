...
import ModalAddLeave from "../../components/attendance/modalAddLeave";
...
const AttendanceByUser = () => {
    ...
    const handleApplyLeave = (record) => {
        setRecordForModal(record);
        handleToogleModal();
    }
    const getAllRecords = async () => {
        await axios.get(`/record/getRecordsByUser/${auth.user.userId}`).then(res => {
            if (res.data.success) {
    ...
    return (
        <>
            ...
            <div className="w-full h-full px-2 sm:px-5 lg:px-10 py-5 bg-zinc-100 min-h-[92vh]">
                <h2 className="text-lg sm:text-3xl font-extrabold text-zinc-500 mb-5">Attendance</h2>
                <div className="flex flex-row justify-between items-center gap-x-2" >
                    <div className="group/search rounded-full h-10  sm:w-6/12 bg-white text-zinc-500 flex flex-row justify-center items-center border-[1px]">
                        {
                            searchTerm === '' &&
                    ...
                    <div className="flex flex-row justify-end items-center gap-x-3 text-white">
                        {/* <button
                            ...
                            disabled={selectedRecords.length == 0 ? true : false} className="font-bold bg-cbaRed px-4 h-10 rounded-full text-xs sm:text-sm flex flex-row justify-center items-center gap-x-2">
                ...
                </div>
                {
                    records.length > 0 ?
                        <div className="overflow-auto">
                            <section className="w-[900px] lg:w-full">
                                <div className="w-full text-xs sm:text-sm font-bold text-zinc-500">
                                    <div className="w-full h-14 bg-blue-50 shadow flex flex-row items-center px-2 rounded-tl-2xl rounded-tr-2xl mt-3 mb-2">
                                        ...
                                        <div className="w-[15%] flex justify-center items-center">Schedule</div>
                                        <div className="w-[10%] flex justify-center items-center">Clock In</div>
                                        <div className="w-[10%] flex justify-center items-center">Clock Out</div>
                                        <div className="w-[10%] flex justify-center items-center">Work Time</div>
                                        <div className="w-[10%] flex justify-center items-center">Late</div>
                                            ...
                                            filteredRecords.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).map((record, index) => (
                                                <div key={index} className="w-full h-16 flex flex-row items-center px-2 bg-white shadow-sm rounded-lg">
                                                    ...
                                                    <div className="w-[5%] flex justify-center items-center">
                                                        <div className="rounded-full bg-orange-50 uppercase h-10 w-10 flex justify-center items-center text-orange-300 ">
                                                    ...
                                                    <div className="w-[15%] flex flex-row justify-center items-center gap-x-2 font-semibold text-zinc-400">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-4">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                        </svg>
                                                        {` ${formatTime(record.onDuty)}-${formatTime(record.offDuty)}`}
                                                    </div>
                                                    <div className={`w-[10%] flex justify-center items-center flex-row gap-x-1.5 font-semibold ${record.clockIn > record.onDuty || record.clockIn == null ? 'text-red-400' : 'text-emerald-400'}`}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-4">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                        </svg>
                                                        {formatTime(record.clockIn)}
                                                    </div>
                                                    <div className={`w-[10%] flex justify-center items-center flex-row gap-x-1.5 font-semibold ${record.clockOut < record.offDuty || record.clockOut == null ? 'text-red-400' : 'text-emerald-400'}`}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-4">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                        </svg>
                                                        <span className={``}>
                                                    ...
                                                    <div className="w-[10%] flex justify-center items-center flex-row gap-x-1.5 font-semibold text-zinc-400">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-4">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                        </svg>
                                                    ...
                                                    <div className="w-[10%] h-full p-[1px] flex items-center justify-center gap-x-1.5 text-zinc-400">
                                                        {
                                                            record.clockOut < record.offDuty || record.clockOut == null ?
                                                                <button
                                                                    onClick={() => handleApplyLeave(record)}
                                                                    className="text-white transition ease-in-out delay-150 duration-300 py-2 px-3 bg-cbaBlue font-semibold text-xs rounded-lg">
                                                                    Apply leave
                                                                </button> :
                                                                record.clockIn > record.onDuty || record.clockIn == null ?
                                                                    <button
                                                                        onClick={() => handleApplyLeave(record)}
                                                                        className="text-white transition ease-in-out delay-150 duration-300 py-2 px-3 bg-cbaBlue font-semibold text-xs rounded-lg">
                                                                        Apply leave
